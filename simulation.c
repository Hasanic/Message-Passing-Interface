	...
	)
{
	...
	float* p0Tmp = (float*)calloc(processCount,sizeof(float)); /* helper array */
	for(i = rank*sliceSize+1; i < (rank+1)*sliceSize+1; ++i) {
        for (j=1; j<=jmax; j++) {
            if (flag[i][j] & C_F) { p0Tmp[rank] += p[i][j]*p[i][j]; }
        }
    }
	// merge p0Tmp array
	timeTmp = MPI_Wtime();
	...
	*ptransTime += MPI_Wtime() - timeTmp;
    ...
    for (iter = 0; iter < itermax; iter++) {
        ...
        *res = 0.0;
		float* resTmp = (float*)calloc(processCount,sizeof(float)); /* helper array */
		for(i = rank*sliceSize+1; i < (rank+1)*sliceSize+1; ++i) {
            for (j = 1; j <= jmax; j++) {
                if (flag[i][j] & C_F) {
                    /* only fluid cells */
                    ...
                    resTmp[rank] += add*add; /* save it here */
                }
            }
        }
		// merge resTmp array

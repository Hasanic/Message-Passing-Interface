...
int main(int argc, char *argv[])
{
	...
	int rank;
	int processCount;
	int sliceSize; /* columns count in a slice */
	double mpiTime = 0;
	double transTime = 0;
	...
	// i used a different variable
	proc = rank;
	nprocs = processCount;

	// check condition
	if(!0 == imax % processCount)
	{
		if(0 == rank)printf(
			"imax = %d,processCount = %d\n"
			"error:imax mod processCount must be 0!\n",
				 imax, processCount );
	...
	if(0 == rank)
		printf(
				"MPI time used in main loop:%e \n"
				"Trans time used in poisson:%e \n",
				mpiTime,transTime);
